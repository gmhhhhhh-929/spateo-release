...
@register_function(
    ...
    auto_fix="none",
    examples=[
        "adata = st.io.spatial.read_visium_hd_bin('outs/binned_outputs/square_016um')",
        "adata = st.io.spatial.read_visium_hd_bin('outs', binsize=16, sample='sample1')",
    ],
...
@register_function(
    ...
    auto_fix="none",
    examples=[
        "adata = st.io.spatial.read_visium_hd_seg('outs/segmented_outputs')",
        "adata = st.io.spatial.read_visium_hd_seg('outs/segmented_outputs', sample='tumor_A')",
    ],
...
def read_visium_hd_seg(
    ...
    # 写入标记，供 st.read_h5ad 读取时自动重建 GeoDataFrame
    adata.uns["spateo_io"] = {"type": "visium_hd_seg", "sample": sample}
    _progress(f"Done (n_obs={adata.n_obs}, n_vars={adata.n_vars})", level="success")
    return adata
...
@register_function(
    ...
    auto_fix="none",
    examples=[
        "adata_bin = st.io.spatial.read_visium_hd('outs', data_type='bin', binsize=16)",
        "adata_cell = st.io.spatial.read_visium_hd('outs/segmented_outputs', data_type='cellseg')",
    ],
...
@register_function(
    ...
    auto_fix="none",
    examples=[
        "st.io.spatial.write_visium_hd_cellseg(cdata, 'output/segmented_outputs')",
    ],
