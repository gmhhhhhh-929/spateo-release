...
def _adaptive_mask(values: np.ndarray, lower: bool, upper: bool, nmads: float) -> np.ndarray:
    ...
    # 1.4826 makes MAD comparable to a standard deviation under a normal model.
    mad = 1.4826 * np.nanmedian(np.abs(values - median))
...
def filter_spots(
    ...
    nmads: float = 3.0,
    exclude_local_outliers: bool = False,
    ...
    if use_in_tissue:
        if in_tissue_key not in adata.obs:
            logger.warning(
                f"`use_in_tissue=True` but `adata.obs[{in_tissue_key!r}]` is missing; skipping tissue filter."
            )
        else:
    ...
    if adaptive:
        ...
        for group in group_values:
            ...
            # High-count spatial regions can be genuine tissue structure, so
            # upper-tail filtering is opt-in instead of an adaptive default.
            adaptive_mask[idx] &= _adaptive_mask(total[idx], lower=True, upper=adaptive_upper, nmads=nmads)
            adaptive_mask[idx] &= _adaptive_mask(genes[idx], lower=True, upper=False, nmads=nmads)
        mask &= adaptive_mask
    if exclude_local_outliers:
        if SKM.OBS_LOCAL_QC_OUTLIER_KEY not in adata.obs:
            raise KeyError("`exclude_local_outliers=True` requires `flag_local_qc_outliers` to run first.")
        mask &= ~np.asarray(adata.obs[SKM.OBS_LOCAL_QC_OUTLIER_KEY], dtype=bool)
    ...
    _record_step(
        ...
        "filter_spots",
        {
            ...
            "nmads": nmads,
            "exclude_local_outliers": exclude_local_outliers,
