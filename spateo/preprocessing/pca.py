...
def pca(
    ...
    if max_components < 1:
        ...
        adata.varm["PCs"] = np.zeros((adata.n_vars, 0), dtype=float)
    ...
    algorithm = "pca"
    if sparse.issparse(X_use) and zero_center:
        dense_bytes = int(X_use.shape[0]) * int(X_use.shape[1]) * np.dtype(np.float32).itemsize
        if dense_bytes <= max_dense_bytes:
            X_use = X_use.toarray().astype(np.float32, copy=False)
        else:
            logger.warning(
                "Centered PCA would require approximately "
                f"{dense_bytes / 1024**3:.2f} GiB; using uncentered TruncatedSVD instead."
            )
            zero_center = False
    if sparse.issparse(X_use) or not zero_center:
