...
def _technical_gene_mask(var_names: object, prefixes: Sequence[str]) -> np.ndarray:
...
def select_spatial_features(
    ...
    valid_methods = {"hvg", "svg", "hvg_svg_union", "hvg_svg_intersection", "all"}
    ...
    hvg_nbatches = np.zeros(adata.n_vars, dtype=int)
    hvg_rank = np.full(adata.n_vars, np.nan, dtype=float)
    if batch_key is not None:
        if batch_key not in adata.obs:
            raise KeyError(f"`batch_key={batch_key!r}` is not present in `adata.obs`.")
        ...
        if batch_scores:
            disp_norm = np.mean(np.vstack(batch_scores), axis=0)
    hvg = _top_mask(disp_norm, n_top_genes, allowed=allowed)
    ranked = np.where(allowed)[0][np.argsort(disp_norm[allowed])[::-1]]
    hvg_rank[ranked] = np.arange(ranked.size, dtype=float)
    adata.var[SKM.VAR_HIGHLY_VARIABLE_KEY] = hvg.astype(bool)
    adata.var["highly_variable_nbatches"] = hvg_nbatches
