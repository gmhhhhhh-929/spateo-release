"""
Function Registry System for Spateo
...
class FunctionRegistry:
    """
    Central registry for all decorated functions in Spateo.
...
def find_function(query: str, verbose: bool = False) -> Union[str, None]:
    ...
    >>> import spateo as st
    >>> st.find_function("质控")
    >>> st.find_function("differential expression")
    >>> st.find_function("pca")
    """
...
def list_functions(category: Optional[str] = None) -> None:
    ...
    --------
    >>> import spateo as st
    >>> st.list_functions()  # List all functions
    >>> st.list_functions("preprocessing")  # List preprocessing functions
    """
    if category:
...
def recommend_function(task_description: str, n: int = 5) -> List[Callable]:
    ...
    --------
    >>> import spateo as st
    >>> st.recommend_function("我想过滤低质量细胞")
    >>> st.recommend_function("perform dimensionality reduction")
    """
...
def export_registry(filepath: Optional[str] = None, 
    ...
    export_metadata = {
        ...
        'spateo_version': getattr(__import__('spateo'), '__version__', 'unknown')
    }
